...
def test_stub_and_convolution_possibility():
...
def test_convolution_filter_integral(v_filter):
    np.testing.assert_allclose(
        ac.get_photometry(ac.vega_CALSPEC, v_filter)[0], (ac.vega_CALSPEC * v_filter).integrate(), rtol=0.01
    )

def test_convolution_filter_set_integral(ubv_filterset):
...
#    def test_vega_system_zero_points(v_filter, ubv_filterset):
#        # TODO: check the agreement percent
#        np.testing.assert_allclose(ac.get_photometry(ac.vega_CALSPEC, v_filter)[0], 3.62708e-11, rtol=0.0025)
#        np.testing.assert_allclose(
#            ac.get_photometry(ac.vega_CALSPEC, ubv_filterset).spectral_dist,
